import { FileText, Clock, CheckCircle, XCircle, AlertCircle, Calendar, MapPin, Phone, Mail, User, Edit, Trash2 } from 'lucide-react';
  ...
  const solicitudes = [
    {
      ...
      priority: 'high',
      notes: 'Solicitud aprobada. Favor de traer identificación y comprobante de domicilio para la visita.'
    },
    {
      id: '2',
      pet: {
        name: 'Max',
        image: 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&q=80',
        breed: 'Labrador Mix',
        age: '2 años'
      },
      refugio: {
    ...
    },
    {
      ...
      pet: {
        name: 'Rocky',
        image: 'https://images.unsplash.com/photo-1561037404-61cd46aa615b?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&q=80',
        breed: 'Pastor Alemán',
        age: '3 años'
      },
      refugio: {
      ...
      priority: 'low',
      notes: 'No cumple con los requisitos de espacio. Se sugiere considerar una mascota más pequeña.'
    },
    {
      id: '4',
      pet: {
      ...
      priority: 'high',
      notes: 'Entrevista telefónica el 26 de enero a las 10:00 AM. Preparar preguntas sobre experiencia con mascotas.'
    }
  ];
    ...
    <div className="ml-64 px-8 pb-10">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">Seguimiento completo del proceso de adopción para cada mascota que has solicitado</p>
        </div>

        {/* Resumen de estadísticas */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
          <div className="text-2xl font-bold text-gray-900 mb-1">{solicitudes.length}</div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
          <div className="text-2xl font-bold text-yellow-600 mb-1">
            {solicitudes.filter(s => s.status === 'pending' || s.status === 'interview').length}
          </div>
        ...
        {solicitudes.length > 0 ? (
          ...
          {solicitudes.map((solicitud) => (
            <div key={solicitud.id} className={`bg-white rounded-lg shadow-sm border-l-4 ${getPriorityColor(solicitud.priority)} border-r border-t border-b border-gray-200 overflow-hidden`}>
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4">
                    ...
                    />
                    <div>
                      <h3 className="text-xl font-semibold text-gray-900 mb-1">{solicitud.pet.name}</h3>
                      <p className="text-gray-600 mb-1">{solicitud.pet.breed} • {solicitud.pet.age}</p>
                      <p className="text-sm text-gray-500">Refugio: {solicitud.refugio.name}</p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-3">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${getStatusColor(solicitud.status)}`}>
                      {getStatusIcon(solicitud.status)}
                      <span className="text-sm font-medium">{getStatusText(solicitud.status)}</span>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2 text-gray-600">
                        <Calendar size={14} />
                        <span>Enviada: {new Date(solicitud.dateSubmitted).toLocaleDateString('es-ES')}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                        <Clock size={14} />
                        <span>Actualizada: {new Date(solicitud.dateUpdated).toLocaleDateString('es-ES')}</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-900">Contacto del Refugio</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2 text-gray-600">
                        <Mail size={14} />
                        <a href={`mailto:${solicitud.refugio.contact}`} className="hover:text-orange-600 transition-colors">
                          {solicitud.refugio.contact}
                        </a>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                        <Phone size={14} />
                        <a href={`tel:${solicitud.refugio.phone}`} className="hover:text-orange-600 transition-colors">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No tienes solicitudes de adopción</h3>
          <p className="text-gray-600 mb-6">
            Cuando envíes una solicitud de adopción, aparecerá aquí para que puedas hacer seguimiento.
          </p>
          <Link 
            to="/adopt" 
        ...
        {solicitudes.length > 0 && (
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-4">📋 Proceso de Adopción</h3>
