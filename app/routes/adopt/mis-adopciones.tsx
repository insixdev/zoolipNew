import { Heart, Clock, CheckCircle, XCircle, FileText, Eye, MessageSquare, Home, Check, X } from 'lucide-react';
...
export default function MisAdopciones() {
  const adopciones = [
    ...
    },
    {
      ...
      pet: {
        name: 'Rocky',
        image: 'https://images.unsplash.com/photo-1561037404-61cd46aa615b?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80',
        breed: 'Pastor Alemán'
      },
      status: 'rejected',
      date: '2024-01-10',
  ...
  const getProgressSteps = (status: string) => {
    const steps = [
      ...
      { id: 5, name: status === 'rejected' ? 'Rechazada' : 'Aprobada', icon: status === 'rejected' ? X : Check }
    ];
    ...
    <div className="mx-auto max-w-6xl md:pl-72 px-4 pt-8 pb-10">
      <div className="mb-12 text-center">
        ...
        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
          Seguimiento completo de tus solicitudes de adopción y el progreso de cada proceso
        </p>
      </div>
      ...
      <div className="space-y-8">
        {adopciones.map((adopcion) => {
          ...
          return (
            <div key={adopcion.id} className="bg-gradient-to-br from-white to-orange-50 rounded-2xl shadow-lg border border-orange-100 p-6 lg:p-8">
              ...
              <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6 mb-6 lg:mb-8">
                ...
                <div className="flex-1 w-full">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
                    <div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{adopcion.pet.name}</h3>
                      <p className="text-base sm:text-lg text-gray-600">{adopcion.pet.breed}</p>
                      <div className="flex items-center gap-1 mt-1">
                        <Home size={14} className="text-teal-500" />
                        <p className="text-sm text-gray-500">{adopcion.refugio}</p>
                      </div>
                    </div>
                    
                    <span className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-semibold border flex items-center gap-2 ${getStatusColor(adopcion.status)}`}>
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <span>Solicitud enviada: {new Date(adopcion.date).toLocaleDateString('es-ES', { 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
                    })}</span>
              ...
              <div className="mb-6 lg:mb-8">
                <h4 className="text-base sm:text-lg font-semibold text-gray-900 mb-4 sm:mb-6">Progreso de Adopción</h4>
                <div className="relative">
                  {/* Línea de progreso */}
                  <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 h-1 bg-gray-200 rounded-full">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        adopcion.status === 'approved' ? 'bg-gradient-to-r from-blue-500 via-purple-500 via-indigo-500 via-teal-500 to-green-500' :
                        adopcion.status === 'rejected' ? 'bg-gradient-to-r from-blue-500 via-purple-500 to-red-500' :
                        currentStep === 1 ? 'bg-blue-500' :
                        currentStep === 2 ? 'bg-gradient-to-r from-blue-500 to-purple-500' :
                        currentStep === 3 ? 'bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-500' :
                  ...
                  <div className="flex justify-between relative">
                    {steps.map((step, index) => {
                      ...
                      const isRejected = adopcion.status === 'rejected' && step.id === 5;
                      const IconComponent = step.icon;
                      
                      // Colores específicos para cada paso
                      const getStepColors = () => {
                        if (isRejected) return 'bg-red-500 border-red-300 text-white';
                        if (isCompleted) {
                          switch (step.id) {
                            case 1: return 'bg-blue-500 border-blue-300 text-white';
                            case 2: return 'bg-purple-500 border-purple-300 text-white';
                            case 3: return 'bg-indigo-500 border-indigo-300 text-white';
                            case 4: return 'bg-teal-500 border-teal-300 text-white';
                            case 5: return 'bg-green-500 border-green-300 text-white';
                            default: return 'bg-orange-500 border-orange-300 text-white';
                          }
                        }
                        if (isCurrent) {
                          switch (step.id) {
                            case 1: return 'bg-blue-100 border-blue-300 text-blue-600';
                            case 2: return 'bg-purple-100 border-purple-300 text-purple-600';
                            case 3: return 'bg-indigo-100 border-indigo-300 text-indigo-600';
                            case 4: return 'bg-teal-100 border-teal-300 text-teal-600';
                            case 5: return 'bg-green-100 border-green-300 text-green-600';
                            default: return 'bg-orange-100 border-orange-300 text-orange-600';
                          }
                        }
                      ...
                      return (
                        <div key={step.id} className="flex flex-col items-center">
                          <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border-2 sm:border-4 transition-all duration-300 ${getStepColors()}`}>
                            <IconComponent size={16} className="sm:w-5 sm:h-5" />
                          </div>
                          <p className={`text-xs mt-1 sm:mt-2 text-center max-w-16 sm:max-w-20 font-medium leading-tight ${
      ...
      {adopciones.length === 0 && (
        <div className="text-center py-16 bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl border border-orange-200">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-3">No tienes solicitudes de adopción</h3>
          <p className="text-lg text-gray-600 mb-8 max-w-md mx-auto">¡Explora nuestras mascotas disponibles y encuentra tu compañero perfecto!</p>
